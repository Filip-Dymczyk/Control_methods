...
public:
    ...
    update() override
    {
        set_value((time() >= m_parameters.start_time) ? m_parameters.scaler : 0.0);
        update_timer();
    }
...
public:
    ...
    update() override
    {
        set_value(is_on() ? m_parameters.scaler : 0.0);
...
private:
    ...
    is_on()
    {
        return (time() >= m_parameters.start_time && m_on_timer <= m_on_time);
    }

    void
    update_on_timer()
    {
        m_on_timer += get_time_step();
    }
};
...
public:
    Sine_Wave(double time_step, double omega, double offset, Signal_Basic_Parameters const& parameters)
        : Signal_Base(time_step, parameters), m_omega(omega), m_offset(offset)
    {
    }
    ...
    update() override
    {
        set_value((time() >= m_parameters.start_time) ? (m_parameters.scaler * sin(m_omega * time()) + m_offset) : 0.0);
        update_timer();
    }
...
public:
    Pulse_Wave(double time_step, double duty_cycle, double period, Signal_Basic_Parameters const& parameters)
        : Rectangle(time_step, duty_cycle * period, parameters), m_period(period)
    {
    }
    ...
    set_duty_cycle(double duty_cycle)
    {
        ...
        Rectangle::set_on_time(duty_cycle * m_period);
    }
