...
void
Inputs_Parser::parse_line_edit_id(QLineEdit* line_edit, LineEdit_ID line_edit_id)
{
    ...
    switch(line_edit_id)
    {
        case LineEdit_ID::INITIAL_CONDITIONS_LINE_EDIT:
        {
            ...
            m_inputs.set_initial_conditions(initial_conditions);
            break;
        }
        case LineEdit_ID::OBJECT_PARAMETERS_LINE_EDIT:
        {
            std::vector<double> const object_parameters = parse_vector_parameters(line_edit);
            m_inputs.set_order(object_parameters.size(), Object_Representation::EQUATION);
            m_inputs.set_object_parameters(object_parameters);
            break;
        }
        ...
        case LineEdit_ID::MEASUREMENT_NOISE_LINE_EDIT:
        {
            ...
            m_inputs.set_measurement_noise_std(measurement_noise_std);
            break;
        }
        case LineEdit_ID::PID_DERIVATIVE_FILTERING_COEFFICIENT_LINE_EDIT:
        {
            ...
            m_inputs.set_pid_derivative_filtering_coefficient(filtering_coefficient);
            break;
        }
        case LineEdit_ID::RLS_FORGETTING_FACTOR_LINE_EDIT:
        {
            ...
            m_inputs.set_RLS_forgetting_factor(RLS_forgetting_factor);
            break;
        }
        default:
            break;
...
void
Inputs_Parser::parse_combobox_id(QComboBox* combobox, ComboBox_ID combo_box_id)
{
    ...
    switch(combo_box_id)
    {
        ...
        {
            m_inputs.set_object_representation(static_cast<Object_Representation>(current_index));
            break;
        }
        case ComboBox_ID::CONTROL_MODE:
        {
            m_inputs.set_control_mode(static_cast<Control_Mode>(current_index));
            break;
        }
        case ComboBox_ID::CONTROLLER_TYPE:
        {
            ...
            m_inputs.set_controller_type(new_controller_type);
            Q_EMIT controller_type_changed(m_inputs.get_controller_parameters());
            break;
        }
        case ComboBox_ID::INPUT_SIGNAL:
        {
            m_inputs.set_input_signal(static_cast<Input_Signal>(current_index));
            Q_EMIT input_signal_changed(m_inputs.get_input_signal_basic_parameters());
            break;
        }
        case ComboBox_ID::OPERATION_TYPE:
        {
            m_inputs.set_operation_type(static_cast<Operation_Type>(current_index));
            break;
        }
        case ComboBox_ID::SIMULATION_TIME_STEP:
        {
            ...
            m_inputs.set_simulation_time_step(value);
            break;
        }
        default:
        {
...
std::vector<double>
Inputs_Parser::parse_vector_parameters(
    QLineEdit* line_edit, int parameters_count, bool parsing_matrix, QString splitter)
{
    assert(line_edit != nullptr);
    ...
    bool ok;
    std::size_t idx = 0u;
    for(QString const& parameter: parameters_string_list)
    {
...
Inputs_Parser::parse_matrix_parameters(QLineEdit* line_edit)
{
    assert(line_edit != nullptr);
    ...
    {
        QLineEdit text_line_edit(text, nullptr);
...
double
Inputs_Parser::parse_single_number_line_edit(QLineEdit* line_edit, double lower_limit, double upper_limit)
{
    bool ok;
...
void
Inputs_Parser::set_plot_control_signal(bool checked)
{
    m_inputs.set_plot_control_signal(checked);
}

void
Inputs_Parser::set_enable_measurement_noise(bool checked)
{
    m_inputs.set_enable_measurement_noise(checked);
}

void
Inputs_Parser::set_enable_pid_derivative_filtering(bool checked)
{
    m_inputs.set_enable_pid_derivative_filtering(checked);
}

Input_Parameters_Container const&
Inputs_Parser::get_input_parameters() const
{
    return m_inputs;
}

void
Inputs_Parser::check_disable_enable_controller_parameters(Controller_Type const& new_controller_type)
{
    Controller_Type const previous_controller_type = m_inputs.get_controller_type();
    if((previous_controller_type == Controller_Type::PID || previous_controller_type == Controller_Type::BANG_BANG) &&
...
QString
Inputs_Parser::get_new_line_edit_text_from_parameters(std::vector<double> const& parameters)
{
