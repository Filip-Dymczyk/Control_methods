...
Main_Window::Main_Window() : m_main_widget(new Main_Widget(this)), m_simulator(new Simulator(this))
{
    connect(
        m_simulator, &Simulator::update_pid_parameters_line_edit_after_tuning, m_main_widget,
...
Main_Window::centralize()
{
    ...
    m_center_position = std::make_tuple(x, y);
...
{
    if(!m_main_widget->dependency_handler()->get_input_parameters().allowed_to_run())
    {
        QMessageBox::critical(this, "Error", "Some inputs have invalid sizes!");
        return;
    }
    m_simulator->update(m_main_widget->dependency_handler()->get_input_parameters());
    m_simulator->run();
}
...
Main_Window::center()
{
    if(std::tuple_size<decltype(m_center_position)>::value == 2)
    {
        this->move(std::get<0>(m_center_position), std::get<1>(m_center_position));
    }
}
...
Main_Window::create_toolbar()
{
    ...
    connect(
        plot_control_signal_checkbox, &QCheckBox::stateChanged, m_main_widget,
    ...
    connect(
        measurement_noise_checkbox, &QCheckBox::stateChanged, m_main_widget, &Main_Widget::enable_measurement_noise);
    ...
    m_main_widget->set_up_line_edit(
    ...
    m_main_widget->set_line_edit_id(static_cast<int>(line_edit_id) + 1);
    ...
    connect(
        pid_derivative_filtering_checkbox, &QCheckBox::stateChanged, m_main_widget,
    ...
    m_main_widget->set_up_line_edit(RLS_forgetting_factor_line_edit, "0.99", tooltip);
    ...
    connect(
        m_main_widget, &Main_Widget::check_tuning_enabled,
        [this, RLS_forgetting_factor_label_action, RLS_forgetting_factor_action]() {
            bool const tuning_enabled = m_main_widget->is_tuning_enabled();
