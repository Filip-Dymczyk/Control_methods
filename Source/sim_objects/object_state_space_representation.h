...
// A is n x n matrix, B is n x 1 vector, C is 1 x n transposed vector D is a scalar (default equals to 0).
...
public:
    ...
    ObjectStateSpaceRepresentation(double time_step, MatrixT const & A, VectorT const & B, VectorT const & C, double D, initial_state const & init_state) : SimumlationObjectBase(time_step), _state(time_step, init_state), _A(A), _B(B), _C(C), _D(D) {}
